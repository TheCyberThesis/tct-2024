...
void backdoor(){
    ...
    num_buf[strcspn(num_buf, "\n")] = '\0';

    char fmt_buf[100];

    if (is_number(num_buf)) {
        int num = atoi(num_buf);
        snprintf(fmt_buf, sizeof(fmt_buf), "Your pattern: %%%d$llx\n", num);
    } else {
        snprintf(fmt_buf, sizeof(fmt_buf), "Your pattern: %s\n", num_buf);
    }
...
void imp(void *address) {
    __asm__ volatile (
        "mov %%rbp, %%rdi\n\t"  // Move the value in rbp to rdi
        "ret\n\t"               // Return
        :                       // No output operands
        :                       // No input operands
        : "%rdi"                // Clobbers the rdi register
    );
}
