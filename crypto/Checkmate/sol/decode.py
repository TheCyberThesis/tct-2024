...
def decode(pgn_string: str, output_file_path: str):
    ...
    with open(output_file_path, "w") as output_file:
        output_file.write("")  # Clear the file before writing
    ...
    for game_index, game in enumerate(games):
        ...
        for move_index, move in enumerate(game_moves):
            ...
            if len(output_data) % 8 == 0:
                output_file.write(
                    bytes([int(output_data[i * 8: i * 8 + 8], 2) for i in range(len(output_data) // 8)])
                )
                output_data = ""

    # If there is leftover binary data, flush it to the file
...
# Load the PGN file and decode it
pgn_file_path = "output.pgn"
with open(pgn_file_path, "r") as pgn_file:
    pgn_string = pgn_file.read()

# Decode the PGN and save the binary data to a new file
